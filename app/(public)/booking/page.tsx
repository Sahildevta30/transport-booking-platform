...
export default async function BookingPage({ searchParams }: Props) {
  ...
  const [{ data: route }, { data: vehicle }] = await Promise.all([
    ...
    supabase
      .from("vehicles")
      .select("id,label,registration_number,vehicle_type_id,status")
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 px-4 py-8 sm:px-6 sm:py-12">
      ...
      <div>
        ...
        <h1 className="text-4xl font-black tracking-tight sm:text-5xl">
          {names.get(route.origin_location_id) ?? "Origin"} →{" "}
          {names.get(route.destination_location_id) ?? "Destination"}
        </h1>
        <p className="mt-2 text-muted-foreground">
          {new Date(trip.departure_at).toLocaleString()} · {vehicle.label} ·
          From ₹{Number(trip.base_price).toFixed(2)}
        </p>
      </div>
      <BookingForm
        tripId={trip.id}
