      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Manage origin, destination, distance and scheduled corridor data.
          </p>
        </div>
        <Button asChild>
          <Link href="/admin/routes/new">Add route</Link>
        </Button>
      </div>
      {error ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {routes.map((route) => {
                  ...
                  const destination = locationMap.get(
                  ...
                  return (
                    <tr key={route.id} className="border-b last:border-0">
                      ...
                      <td className="px-4 py-3">
                        {origin
                          ? `${origin.name}${origin.city ? `, ${origin.city}` : ""}`
                          : "—"}
                      </td>
                      <td className="px-4 py-3">
                        {destination
