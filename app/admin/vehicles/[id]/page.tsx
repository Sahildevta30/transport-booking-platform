...
export default async function VehiclePage({
  ...
  const [{ data: vehicle }, { data: types }, { data: seats }] =
    await Promise.all([
      supabase
        .from("vehicles")
        .select(
          "id,organization_id,vehicle_type_id,label,registration_number,seat_capacity,status",
        )
        .eq("id", id)
        .maybeSingle(),
      supabase
  ...
  async function updateVehicle(formData: FormData) {
    ...
    const parsed = vehicleSchema.safeParse({
      ...
      vehicleTypeId: formData.get("vehicle_type_id"),
      label: formData.get("label"),
      registrationNumber: formData.get("registration_number"),
      seatCapacity: formData.get("seat_capacity") ?? "",
      status: formData.get("status"),
    });
    if (!parsed.success) redirect(`/admin/vehicles/${id}?error=invalid`);
    ...
    if (error) redirect(`/admin/vehicles/${id}?error=save`);
    redirect("/admin/vehicles");
  }
  async function addSeats(formData: FormData) {
    "use server";
    const parsed = seatBatchSchema.safeParse({
      vehicleId: id,
      prefix: formData.get("prefix"),
      count: formData.get("count"),
      seatType: formData.get("seat_type"),
    });
    if (!parsed.success) redirect(`/admin/vehicles/${id}?error=seats`);
    const client = await createClient();
    const s = parsed.data;
    const rows = Array.from({ length: s.count }, (_, index) => ({
      vehicle_id: id,
      seat_number: `${s.prefix.toUpperCase()}${index + 1}`,
      seat_type: s.seatType || "standard",
    }));
    const { error } = await client
      .from("vehicle_seats")
      .upsert(rows, {
        onConflict: "vehicle_id,seat_number",
        ignoreDuplicates: true,
      });
    if (error) redirect(`/admin/vehicles/${id}?error=seats`);
    ...
    <div className="mx-auto max-w-4xl space-y-7">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4 rounded-2xl border bg-card p-6 shadow-card">
          <div>
            <h2 className="font-semibold">Seat layout</h2>
            <p className="text-sm text-muted-foreground">
              {seats?.length ?? 0} seats configured
            </p>
          </div>
          <form action={addSeats} className="grid grid-cols-3 gap-3">
            <Input name="prefix" placeholder="Prefix A" required />
            <Input
              name="count"
              type="number"
              min={1}
              ...
              required
            />
            <Input name="seat_type" placeholder="standard" />
          ...
          <div className="flex flex-wrap gap-2">
            {seats?.map((seat) => (
              <span
                ...
                title={seat.seat_type ?? undefined}
              >
                {seat.seat_number}
              </span>
            ))}
