...
export default async function NotificationsPage() {
  ...
  if (!user) redirect("/login?next=/customer/notifications");
  const { data: items, error } = await supabase
    .from("notifications")
    .select(
      "id,booking_id,channel,event_type,title,message,status,read_at,sent_at,created_at",
    )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Booking, payment, refund and trip updates.
        </p>
      </div>
      {error ? (
        ...
        <div className="space-y-3">
          {items.map((n) => (
            <article
              key={n.id}
              className="relative rounded-2xl border bg-card p-5 shadow-card"
            >
              {!n.read_at ? (
                <span
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-xs">
                <span>{n.channel.replaceAll("_", " ")}</span>
                <span>{n.status}</span>
                {n.booking_id ? (
                  <Link
                    className="underline"
                    href={`/customer/bookings/${n.booking_id}`}
                  >
                    View booking
                  </Link>
                ) : null}
